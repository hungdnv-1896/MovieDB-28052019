...
class FSPagerViewLayout: UICollectionViewLayout {
    ...
    override open func prepare() {
        guard let collectionView = self.collectionView,
            ...
            self.needsReprepare || self.collectionViewSize != collectionView.frame.size else {
            return
        }
        ...
        self.itemSpacing = (self.scrollDirection == .horizontal ?
            self.actualItemSize.width : self.actualItemSize.height)
            + self.actualInteritemSpacing
    ...
    override open func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        while origin - maxPosition <= max(CGFloat(100.0) * .ulpOfOne * abs(origin + maxPosition),
        ...
        func calculateTargetOffset(by proposedOffset: CGFloat, boundedOffset: CGFloat) -> CGFloat {
            ...
            if pagerView.decelerationDistance == FSPagerView.automaticDistance {
                ...
                case 0.3 ... CGFloat.greatestFiniteMagnitude:
                    targetOffset = ceil(collectionView.contentOffset.x / self.itemSpacing+CGFloat(extraDistance))
                        * self.itemSpacing
                case -CGFloat.greatestFiniteMagnitude ... -0.3:
                    targetOffset = floor(collectionView.contentOffset.x / self.itemSpacing-CGFloat(extraDistance))
                        * self.itemSpacing
                default:
                    targetOffset = round(proposedOffset/self.itemSpacing) * self.itemSpacing
                }
    ...
    fileprivate func adjustCollectionViewBounds() {
        ...
        let newIndexPath = IndexPath(item: currentIndex,
                                     section: pagerView.isInfinite ? self.numberOfSections / 2 : 0)
    ...
    fileprivate func applyTransform(to attributes: FSPagerViewLayoutAttributes,
                                    with transformer: FSPagerViewTransformer?) {
        guard let collectionView = self.collectionView else {
            return
        }
