...
open class FSPageControl: UIControl {
    ...
    open override func layoutSublayers(of layer: CALayer) {
        ...
        var x: CGFloat = {
            ...
            case .center, .fill:
                ...
                let amplitude = CGFloat(self.numberOfPages / 2) * diameter +
                    spacing * CGFloat((self.numberOfPages - 1) / 2)
                return midX - amplitude
            case .right, .trailing:
                let contentWidth = diameter * CGFloat(self.numberOfPages) +
                    CGFloat(self.numberOfPages - 1) * spacing
    ...
    fileprivate func updateIndicatorsIfNecessary() {
        guard self.needsUpdateIndicators, self.indicatorLayers.count > 0 else {
            return
        }
    ...
    fileprivate func updateIndicatorAttributes(for layer: CAShapeLayer) {
        ...
        if let image = self.images[state] {
            ...
            layer.path = self.paths[state]?.cgPath ??
                UIBezierPath(ovalIn: CGRect(x: 0, y: 0, width: self.itemSpacing, height: self.itemSpacing)).cgPath
        }
